...
/**
 * A cache of mDNS records and responses. The cache obeys TTLs, so items are purged after their validity period is
 * complete. Negative answers are cached, to avoid repeated failed DNS queries. The credibility of each RRset is
 * maintained, so that more credible records replace less credible records, and lookups can specify the minimum
 * credibility of data they are requesting.
 * 
 * !!! This cache implementation extends the Cache class. Reflection had to be used to access !!! private fields and
 * methods in the Cache class that prevent its direct extensibility.
 * 
 * @see Cache
 * 
 * @author Steve Posick
...
public class MulticastDNSCache extends Cache implements Closeable {
    protected static final Logger logger =
        Misc
            .getLogger(MulticastDNSCache.class.getName(),
                Options.check("mdns_verbose") || Options.check("dns_verbose") || Options.check("verbose"));

    /**
     * The CacheCheck interface allows a client to monitor the Cache data and implement a cache refresh mechanism, such
     * as what is required for mDNS Caching. See RFC 6762 Section 5.2
     * 
     * @author Steve Posick
     */
    public static interface CacheMonitor {
        /**
         * Called at the beginning of a Monitor task. Clears the state and prepares the CacheMonitor for the receipt of
         * check and expired calls.
         */
        public void begin();
         ...
         *            The RRset
         * @param credibility
         *            The credibility of the RRset
         * @param expiresIn
         *            The time remaining before the record expires.
         * @return true if the RRset can be removed from the cache
         */
        public void check(RRset rrs, int credibility, int expiresIn);

        /**
         * Called at the end of a Monitor task. Clears the state, releases resources, and performs refresh of items
         * identified as requiring a refresh.
         */
        public void end();
    ...
    protected static class ElementHelper {
        ...
        protected ElementHelper(final Cache cache, final Object element)
            ...
            AccessibleObject.setAccessible(new AccessibleObject[] { expireField, credibilityField }, true);
            AccessibleObject
                .setAccessible(new AccessibleObject[] { expired, compareCredibility, getType, getTTL, expireField,
        ...
        private void processElement(final ElementHelper element) {
            try {
                if (element.getElement() instanceof RRset) {
                    ...
                    logger
                        .logp(Level.INFO, getClass().getName(), "processElement",
                            "Element is an unexpected type \""
                                + (element.getElement() != null ? element.getElement().getClass().getName() : "null")
                                + "\"");
                }
            }
            catch (Exception e) {
    ...
    static {
        ...
        try {
            ...
            logger
                .log(Level.WARNING, "Unrecoverable Error: The base " + Cache.class
     ...
     */
    public MulticastDNSCache(final String file) throws IOException, NoSuchFieldException, NoSuchMethodException {
        super(file);
        populateReflectedFields();
    }
     ...
     * 
     * @param cache
     *            The Cache to use to populate this mDNSCache.
     * @throws NoSuchFieldException
     *             if the "data" field does not exist in either Cache
     * @throws IllegalAccessException
     *             if the "data" field could not be accessed in either Cache
     */
    MulticastDNSCache(final Cache cache) throws NoSuchFieldException, NoSuchMethodException, IllegalAccessException {
        this();
    ...
    /**
     * Acquires additional information from the cache so that the returned results have all the information required in
     * 1 query.
     * 
     * @param name
     *            The root name to begin the query sequence
    ...
    /**
     * Sets the CacheMonitor used to monitor cache data. Can be used to implement Cache record refreshing.
     * 
     * @param monitor
     *            the CacheMonitor
     */
    ...
    /**
     * !!! Work around to access private methods in Cache superclass !!! This method will be removed when the super
     * class is made extensible (private members made protected) Populates the local copies of the needed private super
     * fields.
     * 
     * @throws SecurityException
     * @throws NoSuchFieldException
    ...
    protected void populateReflectedFields() throws NoSuchFieldException, NoSuchMethodException {
        ...
        try {
            ...
            AccessibleObject.setAccessible(new AccessibleObject[] { findElement, removeElement }, true);
        }
        catch (NoSuchMethodException e) {
            logger.log(Level.WARNING, e.getMessage(), e);
            throw e;
