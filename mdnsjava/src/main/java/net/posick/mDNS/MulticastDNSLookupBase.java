...
public abstract class MulticastDNSLookupBase implements Closeable, Constants {
    protected static final Logger logger =
        Misc
            .getLogger(MulticastDNSLookupBase.class.getName(),
    ...
    public MulticastDNSLookupBase(final Name[] names, final int type) throws IOException {
        this(names, type, DClass.ANY);
    }

    public MulticastDNSLookupBase(final Name[] names, final int type, final int dclass) throws IOException {
        this();
                ...
                if (names[index].endsWith(".")) {
                    ...
                    for (int i = 0; i < searchPath.length; i++) {
                        try {
                            ...
                            if (logger.isLoggable(Level.FINE)) {
                                logger
                                    .log(Level.FINE, "Error parsing \"" + (names[index] + "." + searchPath[i]) + "\" - "
                                        + e.getMessage(), e);
                            }
                        }
                    }
    ...
    protected MulticastDNSLookupBase(final Message message) throws IOException {
        this();
        queries = new Message[] { message.clone() };
     ...
     *             If name is invalid
     */
    public void addNames(final String[] names) throws TextParseException {
        if ((names != null) && (names.length > 0)) {
     ...
     *             If name is invalid
     */
    public void addSearchPath(final String[] searchPath) throws TextParseException {
        if ((searchPath != null) && (searchPath.length > 0)) {
     ...
     *            Names to browse
     */
    public void setNames(final String[] names) throws TextParseException {
        if (names == null) {
            this.names = null;
            return;
        }
    ...
    public static synchronized Name[] getDefaultSearchPath() {
        if (defaultSearchPath == null) {
            ...
            defaultSearchPath =
                new Name[(configuredSearchPath != null ? configuredSearchPath.length : 0)
            ...
            System
                .arraycopy(defaultQuerier.getMulticastDomains(), 0, defaultSearchPath, startPos,
                    defaultQuerier.getMulticastDomains().length);
        }
    ...
    protected static ServiceInstance[] extractServiceInstances(final Message... messages) {
        ...
        for (Message message : messages) {
            Record[] temp =
                MulticastDNSUtils.extractRecords(message, Section.AUTHORITY, Section.ANSWER, Section.ADDITIONAL);
            if (records == null) {
                records = temp;
            }
            else {
