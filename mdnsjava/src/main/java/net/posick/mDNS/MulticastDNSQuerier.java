...
/**
 * The MulticastDNSQuerier is a responder that integrates multicast and unicast DNS in accordance to the mDNS
 * specification [RFC 6762]. DNS queries for multicast domains are send as multicast DNS requests, while unicast domain
 * queries are sent as unicast DNS requests.
 * 
 * @author posicks
 */
...
public class MulticastDNSQuerier implements Querier {
    private static final Logger logger =
        Misc.getLogger(MulticastDNSQuerier.class, Options.check("mds_verbose") || Options.check("verbose"));
    ...
    protected static class Resolution implements ResolverListener {
        ...
        public Message getResponse(final int timeout) throws IOException {
        ...
        public Message[] getResults(final boolean waitForResults, final int timeoutValue) throws Exception {
            if (waitForResults) {
        ...
        public void handleException(final Object id, final Exception exception) {
            if ((requestIDs.size() != 0) && (!requestIDs.contains(id) || (this != id) || !equals(id))) {
                logger
                    .logp(Level.FINE, getClass().getName(), "handleException",
        ...
        public void receiveMessage(final Object id, final Message message) {
            if ((requestIDs.size() == 0) || requestIDs.contains(id) || (this == id) || equals(id)
                ...
                logger
                    .logp(Level.FINE, getClass().getName(), "receiveMessage",
        ...
        public Object start() {
            ...
            if (!unicast && !multicast) {
                logger
                    .logp(Level.SEVERE, getClass().getName(), "start",
     ...
     * @param ipv6
     *            if IPv6 should be enabled.
     * @param unicastResolver
     *            The Unicast DNS Resolver
     * @throws UnknownHostException
     */
     ...
     * @param ipv6
     *            if IPv6 should be enabled.
     * @param unicastResolvers
     *            The Unicast DNS Resolvers
     * @throws UnknownHostException
     */
    public MulticastDNSQuerier(final boolean ipv4, final boolean ipv6, final Resolver[] unicastResolvers)
        ...
        if ((ipv4Responder != null) && (ipv6Responder != null)) {
            multicastResponders = new Querier[] { ipv4Responder, ipv6Responder };
    ...
    public void setRetryWaitTime(final int secs) {
        for (Querier querier : multicastResponders) {
            querier.setTimeout(Duration.ofSeconds(secs));
        }
    }
    ...
    public void setRetryWaitTime(final int secs, final int msecs) {
        for (Querier querier : multicastResponders) {
            querier.setTimeout(Duration.ofMillis(secs * 1000 + msecs));
        }
    }
    ...
    public void setTimeout(final int secs, final int msecs) {
        ...
        for (Resolver resolver : unicastResolvers) {
            resolver.setTimeout(Duration.ofMillis(secs * 1000 + msecs));
        }
    }
